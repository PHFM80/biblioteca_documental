...
def section(title, content):
    ...
    """
    return ft.Container(padding=15, border_radius=10, bgcolor=ft.Colors.GREY_100, content=ft.Column(
            controls=[
...
def view(page, router, render):
    ...
    pages_view.load_pages()
    pages_view.set_on_delete(lambda page_number: remove_scanned_page(page_number, page, preview_view, pages_view))
    ...
    return ft.Container(expand=True, padding=30, content=ft.Column(scroll=ft.ScrollMode.AUTO, spacing=15,
            controls=[
                ...
                section("Finalizar", finish_actions(router, render))]   ))