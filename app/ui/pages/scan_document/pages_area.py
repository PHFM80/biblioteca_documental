...
class PagesArea:
    ...
    def __init__(self, on_delete=None):
        ...
        self.container = ft.Row(scroll=ft.ScrollMode.AUTO, spacing=10)

    def load_pages(self):
        self._pages.clear()
        self.container.controls.clear()
        for page in document_session.get_pages():
            self.add_page(page, update=False)

    def add_page(self, page: DocumentPage, update=True):
        """
        Agrega una nueva página visual.
        """
        thumbnail = DocumentThumbnail(page=page, on_delete=self.on_delete, show_actions=True)
        self._pages.append(thumbnail)
        self.container.controls.append(thumbnail.card)
        if update:
            self.update()
    ...
    def _renumber(self):
        ...
        """
        for index, thumbnail in enumerate(self._pages,start=1,):
            thumbnail.set_page_number(index)
